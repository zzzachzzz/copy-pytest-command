...
class CopyPytestCommandCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        full_file_path = self.view.file_name()
        line = self.get_line_of_cursor()
        test_specifier = get_test_specifier(full_file_path, line)
        file_name = re.search('[^/]+.py$', full_file_path).group()
        command = "pytest {}{}".format(file_name, test_specifier)
        print(command)
        sublime.set_clipboard(command)
        self.view.window().status_message("\tCopied command:\t{}".format(command))

    def get_line_of_cursor(self):
        """
        This helper method which locates a cursor/region in given view
        and returns selected/containing test class/method.
        """
        r = self.get_first_selection()
        if r is None:
        ...
        line, col = self.view.rowcol(int(r.a))
        line = line + 1
        assert line, ('No line found in region: %s' % r)
        print('Position in code -> line %s' % line)
        return line
...
def get_test_specifier(module, line):
    # type: (str, int) -> str
    with open(module) as module_file:
        source_code = module_file.read()
        parser = TestParser(source_code)
        result = parser.parse(line=int(line))
        printable_result = ''.join('::' + part for part in result if part)
